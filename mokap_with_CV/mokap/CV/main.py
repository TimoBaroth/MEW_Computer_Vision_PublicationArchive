...
def main(argv):
    ...
    def saveMeasurements(measurement_dict, csv_writer, out_path, verbose):
    ...
    try:
        opts, args = getopt.getopt(argv, "hM:i:I:o:b:e:S:j:vsmtca:d:rFV:l:L:",[])
    ...
    #If we try to measure based on knows shapes
        ...
        if save_measurement:
            ...
            measurement_out_path, measurement_out_csv, measurement_out_csv_writer = createMeasurementOut(out_path) #create output csv/sub-dir
        ...
        while True:
            ...
            if save_measurement:
                saveMeasurements(measurement_dict, measurement_out_csv_writer, measurement_out_path, verbose)
     ...
     #If we try to extract the shapes and at the same time take measurements 
        ...
        while True:
            ...
            if image_loaded: #only if succsesful, continue with shape extraction
                ...
                if save_shape: # if we want to save the output
                    shape_file_name = saveShape(shape, img_key, img_dict[img_key][1], shape_out_path, shape_out_csv_writer, verbose)
