...
class VideoWindowCV(VideoWindowBase): #copied and modified from VideoWindowRec
    newFrameSignal = Signal(np.ndarray)
    
    def __init__(self, main_window_ref, idx):
        super().__init__(main_window_ref, idx)

        self.annotation_checkbox = {}
    ...
    def _update_vars(self): #overwrite parent function =>> other labels etc.
        ann = []
        for key in self.annotation_checkbox:
            if self.annotation_checkbox[key].isChecked(): # if set to True
                ann.append(self._main_window.cv[self.idx].annotations[key]) # add value
        self._main_window.cv[self.idx].annotate = ann #update value list for annotations
        self._main_window.cv[self.idx].text = self.cv_result_checkbox.isChecked()
    ...
    def _init_specific_ui(self):
        ...
        # Camera name bar
        camera_name_bar = QLabel(f'Computer vision {self._camera.name.title()} camera')
        camera_name_bar.setFixedHeight(25)
        camera_name_bar.setAlignment(Qt.AlignCenter)
        camera_name_bar.setStyleSheet(f"color: {self.colour_2}; background-color: {self.colour}; font: bold;")
        ...
        bottom_panel_h_layout.addWidget(self.LEFT_GROUP, 1)     # Expand the group

        main_layout.addWidget(self.BOTTOM_PANEL)
        left_group_layout = QVBoxLayout(self.LEFT_GROUP)

        annotation_groupbox = QGroupBox("Annotation")
        annotation_groupbox.setContentsMargins(5, 20, 0, 5)
        annotation_groupbox_layout = QVBoxLayout(annotation_groupbox)
        annotation_groupbox_layout.setSpacing(2)

        for key in self._main_window.cv[self.idx].annotations:
            checkbox = QCheckBox()
            checkbox.setChecked(False)
            checkbox.setText(f"{key}")
            annotation_groupbox_layout.addWidget(checkbox)
            self.annotation_checkbox[key] = checkbox
        
        checkbox = QCheckBox()
        checkbox.setChecked(False)
        checkbox.setText("Results")
        annotation_groupbox_layout.addWidget(checkbox)
        self.cv_result_checkbox = checkbox

        left_group_layout.addWidget(annotation_groupbox)

    def _refresh_framebuffer(self): #overwrite parent function =>> other framebuffer 
        """
            Grabs a new frame from the cameras and stores it in the frame buffer
        """
        if self._main_window.mc.acquiring:
            arr = self._main_window.cv[self.idx].out_img
            if arr is not None:
                self._frame_buffer = arr
        else:
            self._frame_buffer.fill(0)
...
class MainWindow(QMainWindow):
    ...
    def _start_secondary_windows(self):
        ...
        for i, _ in enumerate(self.cv):
            w = VideoWindowCV(main_window_ref=self, idx=self.cv[i].camidx)
            
            self.secondary_CV_windows.append(w)
            w.show()
    ...
    def _stop_secondary_windows(self):
        ...
        for w in self.secondary_CV_windows:
            w._force_destroy = True
            w.close()
