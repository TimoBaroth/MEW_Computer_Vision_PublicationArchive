...
class SerialTrigger:
    ...
    def __init__(self, silent=False):
        ...
        comport = "/dev/ttyACM0"
        self.serialdevice = serial.Serial(port=comport, baudrate=9600)
...
class MQTTclient:
    def __init__(self):
        ...
        self.cl = mqtt.Client(mqtt.CallbackAPIVersion.VERSION1,protocol=mqtt.MQTTv5)
        self.cl.connect(self.mqtt_ip, self.mqtt_port) 
        self.cl.loop_start()
...
class MQTTLogger:
    ...
    def __init__(self,client):
        ...
        N2_topic1 = "gas_heater_controller_out"
        N2_topic2 = "gas_heater_controller_in"
        ...
        self.client.subscribe([(TCS_topic,2),(TCA_topic,2),(TRS_topic,2),(TRA_topic,2),(PS_topic,2),(PA_topic,2),(HVS_topic,2),(HVA_topic,2),(S_topic,2),(N2_topic1,2),(N2_topic2,2)])
        ...
        self.client.message_callback_add(N2_topic1, self.N21_callback) # add callback for specific topic
        self.client.message_callback_add(N2_topic2, self.N22_callback) # add callback for specific topic
    ...
    def N21_callback(self, client, userdata, message):
        try: 
            d = {}
            d = json.loads(message.payload)
            self.values["N2A"] = float(d["N2_temp_act"])
            self.values["TA"] = float(d["ambient_temp_act"])
            self.values["RHA"] = float(d["ambient_humidity_act"])
        except:
            self.values["N2A"] = -1
            print("N2 actual temp aquisition failed")

    def N22_callback(self, client, userdata, message):
        try: 
            d = {}
            d = json.loads(message.payload)
            self.values["N2S"] = float(d["N2_temp_set"])
        except:
            self.values["N2S"] = -1
            print("N2 set temp aquisition failed")
...
class MQTTWriter:
    ...
    #custom encoder to solve stupid json numpy incompatibility 
    class NpEncoder(json.JSONEncoder): #from: https://stackoverflow.com/questions/50916422/python-typeerror-object-of-type-int64-is-not-json-serializable
    ...
    def publish_msg(self, msg):
        self.client.publish(self.topic, json.dumps(msg, cls=self.NpEncoder), qos=2)
