...
class MultiCam:
    ...
    def __init__(self,
        ...
        self._l_latest_frames_lock: List[Lock] = []
        self._l_CV_buffers_lock: List[Lock] = []
        self._l_CV_buffers_event: List[Event] = []
        self._l_CV_buffers: List[np.array] = []
        ...
        for i, cam in enumerate(self._sources_list):
            ...
            self._l_CV_buffers.append(np.zeros(cam.shape, dtype=np.uint8))
            self._l_CV_buffers_lock.append(Lock())
            self._l_CV_buffers_event.append(Event())
            self._l_finished_saving.append(Event())
            self._l_all_frames.append(deque())
            self._l_latest_frames.append(deque(maxlen=1))
            self._l_latest_frames_lock.append(Lock())
    ...
    def binning(self, value: int) -> None:
        for i, cam in enumerate(self._sources_list):
            ...
            self._l_CV_buffers[i] = np.zeros(cam.shape, dtype=np.uint8)
    ...
    def _display_updater_thread(self, cam_idx: int) -> NoReturn:
        ...
        lock1 = self._l_latest_frames_lock[cam_idx]
        lock2 = self._l_CV_buffers_lock[cam_idx]
        event = self._l_CV_buffers_event[cam_idx]
        timer = Event()

        while self._acquiring:
            timer.wait(1/self._framerate) # 0.05maybe adjust time
            if queue and lock1.acquire(blocking=False):
                np.copyto(self._l_display_buffers[cam_idx], queue.popleft())
                lock1.release()
                self._cnt_displayed[cam_idx] += 1
                if lock2.acquire(blocking=False):
                    np.copyto(self._l_CV_buffers[cam_idx], self._l_display_buffers[cam_idx])
                    if not event.is_set():
                        event.set()
                    lock2.release()
    ...
    def _grabber_thread(self, cam_idx: int) -> NoReturn:
        ...
        queue_mqtt = self._l_mqtt_readings[cam_idx]
        lock = self._l_latest_frames_lock[cam_idx]
        cam.start_grabbing()
        while self._acquiring:
