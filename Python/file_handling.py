...
def listImagesNEW(img_path):
    ...
    img_count = 0
    ok = False
    try: #normally we expect a path to a dict
        file_list = os.listdir(img_path) #get list of files
        ok = True
    except: # if it fails, check if we got a path to a single file 
        try: 
            if os.path.isfile(img_path):
                file_list = []
                file_list.append(os.path.basename(img_path))
                ok = True
        except:
            pass
        pass
    if ok == True: # if we got some files we can work with
