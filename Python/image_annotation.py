...
def drawDiameterLines(cimg, diameters, left_contour, right_contour, contour_intersection_indices, text=True):
    ...
    for i in contour_intersection_indices:
        if i[0] != -1:
            cv.line(cimg, (left_contour[1][i[0]], left_contour[0][i[0]]), (right_contour[1][i[1]], right_contour[0][i[1]]), color=color_diameter_lines, thickness=5, lineType=cv.LINE_AA) #thickness = 5
            if text:
